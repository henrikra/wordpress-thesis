...
Seuraavaksi muutetaan aikaisemmin tehty virtuaalikoneen Ansiblen playbook roolipohjaiseksi. Lisäksi playbookkiin lisätään MongoDB, joka on suosittu tietokantaohjelma Node-projekteissa. Noden ja MongoDB:n asennus jaetaan erillisiin rooleihin, joka helpottaa roolien uudelleenkäyttöä muissa projekteissa.
...
Luodaan hosts-tiedosto ansible-kansioon. Hosts-tiedostossa kerrotaan Ansiblelle kaikkien kohdekoneiden tietoja, kuten ip-osoite. Kohdekoneita voidaan myös ryhmitelmmä hosts-tiedostossa. Esimerkiksi jos tietokantapalvelimia on monta niin ne voi nimetä hosts-tiedostossa esimerkiksi databases-ryhmän alle.

Kuviosta nähdään hosts-tiedoston sisältö. Kuviossa \ref{listing:ansible-hosts} hakasulkeiden sisällä oleva teksti tarkoittaa ryhmää ja sen alapuolella olevat osoitteet ovat kyseisen ryhmän kohdekoneita. Nyt kohdekoneena on vain localhost eli virtuaalikone.
...
Seuraavaksi ansible kansioon lisätään ansible.cfg-tiedosto. Tiedoston sisältö näkyy kuviosta \ref{listing:ansible-config}. Tiedostossa kerrotaan mistä Ansiblen täytyy etsiä hosts-tiedostoa. Oletuksena hosts-tiedostoa etsitään polusta \code{etc/ansible/hosts} \cite{link:ansible-inventory}.
